...
describe("rateLimit", () => {
  ...
  it("allows requests up to the limit", () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(rateLimit(key, 5, 10_000).allowed).toBe(true);
    }
  });
  ...
  it("resets the bucket after the window elapses", () => {
    ...
    for (let i = 0; i < 2; i++) {
      expect(rateLimit(key, 2, 1_000).allowed).toBe(true);
    }
    expect(rateLimit(key, 2, 1_000).allowed).toBe(false);
  ...
  it("tracks separate keys independently", () => {
    ...
    rateLimit(keyA, 1, 10_000);
    expect(rateLimit(keyA, 1, 10_000).allowed).toBe(false);
    expect(rateLimit(keyB, 1, 10_000).allowed).toBe(true);
  });
});
