...
export default function FaqPage() {
  const splitAt = Math.ceil(FAQ_ENTRIES.length / 2);
  const firstHalf = FAQ_ENTRIES.slice(0, splitAt);
  const secondHalf = FAQ_ENTRIES.slice(splitAt);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      {secondHalf.length > 0 ? (
        <>
          <AdSlot placementId={109} className="mt-8" />
